...
<div class="form_group mt-5 text-center">
 <!--  <select class="form-control text-center" name="" id="building"></select> --> 
   <select name="" id="building" onchange="selectedBuilding()"></select> 
...
<script>
  var manager_mail = document.getElementById("manager_email").value;
   ...
   // fetch('https://iqbetspro.com/pg-management/GET-Managers-details-API.php', {
      method: 'POST',
      body: JSON.stringify(
        {
      ...
      .then(function(data)
      {
       ...
       // var option ="<option value=''>Select the Building name</option>";
        ...
        for (var i=0;i<data.length;i++)
        {
             ...
             option+= '<option value="' + i + '">' + data[i].building_name + "</option>"
        }
